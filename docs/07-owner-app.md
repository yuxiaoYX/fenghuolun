...
| 能耗 | `pages/energy` | 电（官方 kWh）和油（油量差分）并列；近 7 日柱图；加油/耗油差分列表。没有花费/充电账单接口则空态，不编钱 |
| 电池 | `pages/battery` | 12V、同电量包电压、电流；SOH 与满电容量估算无证据则空态 |
| 我的 | `pages/more` | 车辆资料、绑定/更换 `refresh_token`、解绑、关于与免责；电价/导出等未开通项占位 |

绑定页 `pages/bind` 不进底栏。远程启动在车况页底部占位「暂未开放」。
...
3. 上报超过 2 小时由后端计算 `stale`（不落库）；车况相对时间与电池时效条变色。无 `reportedAt` 不标陈旧
