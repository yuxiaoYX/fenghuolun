...
| `FENGHUOLUN_SESSION_SECRET` | 会话签名 |
| `FENGHUOLUN_ADMIN_BOOTSTRAP_USER` | 仅当库中无管理员时创建 |
| `FENGHUOLUN_ADMIN_BOOTSTRAP_PASSWORD` | 同上 |
| `FENGHUOLUN_CORS_ORIGINS` | 逗号分隔 |
| `FENGHUOLUN_CRON_SYNC` | 默认关；如 `15m` |
...
| `GET` | `/api/v1/owner/snapshot/latest` | 最新解码快照；另带计算字段 `stale`（上报超过 2 小时） |
...
| `GET` | `/api/v1/admin/bindings/:id` | 详情，无凭证明文 |
| `POST` | `/api/v1/admin/bindings/:id/disable` | 停用 |
| `GET` | `/api/v1/admin/jobs` | 同步任务 |
| `GET` | `/api/v1/admin/health` | 队列、上游连续失败计数 |

管理员请求头与车主不同 audience，中间件分开。
...
3. 换票：接口未证实前，客户端用明确的 `ErrRefreshUnverified`，不要调用猜的 URL 当成成功。
