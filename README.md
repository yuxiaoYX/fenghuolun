...
第 **2** 期已落地：只读同步、SQLite、管理员后台。权威说明只以 `docs/` 为准。
...
| 管理员后台 | [`apps/admin`](apps/admin) | Vue 3 + Vite | 仅管理员 |
| 后端 | [`server`](server) | GoFrame v2 + SQLite（库表从第 1/2 期起） | 换票、拉数、落库、鉴权 |
...
抓包附录只留本机 `docs/HAR-ANALYSIS.md`（已 gitignore，不进仓库）。旧原型稿：[`docs/obsolete/`](docs/obsolete/)。
...
浏览器打开终端里的 `http://127.0.0.1:5173`。先起后端才能在总览里看到 healthz。
...
下一期是只读协议打通，见 [`docs/04-roadmap.md`](docs/04-roadmap.md)。不要做车控。
